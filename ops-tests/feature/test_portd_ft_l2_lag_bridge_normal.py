...
#
# Objective:   Verify Linux bonding interface for L2 LAG is properly added
#              or remove from the bridge_normal. Also verify if L2 interfaces
...
# Check if a given interface is part of bridge_normal
def sw_is_interface_in_bridge(sw, intf_name):
...
def test_lag_bridge_normal_configuration(topology):
    ...
    print("Verify interfaces 1 and 2 are part of bridge_normal")
    for port in ports_sw1:
        assert sw_is_interface_in_bridge(sw1, port),\
            "Interface %s should be part of bridge_normal" % (port)
    for port in ports_sw2:
        assert sw_is_interface_in_bridge(sw2, port),\
            "Interface %s should be part of bridge_normal" % (port)

    print("Create L2 LAG in both switches")
    create_l2_lag(sw1, sw_lag_id)
    create_l2_lag(sw2, sw_lag_id)
    ...
    assert sw_is_interface_in_bridge(sw1, sw_lag_name),\
        "LAG %s should be part of bridge_normal" % (sw_lag_name)
    assert sw_is_interface_in_bridge(sw2, sw_lag_name),\
        "LAG %s should be part of bridge_normal" % (sw_lag_name)

    print("Verify interfaces 1 and 2 are not part of bridge_normal")
    for port in ports_sw1:
        assert not sw_is_interface_in_bridge(sw1, port),\
            "Interface %s should not be part of bridge_normal" % (port)
    for port in ports_sw2:
        assert not sw_is_interface_in_bridge(sw2, port),\
            "Interface %s should not be part of bridge_normal" % (port)
    ...
    assert not sw_is_interface_in_bridge(sw1, sw_lag_name),\
        "L3 LAG %s should not be part of bridge_normal" % (sw_lag_name)
    assert not sw_is_interface_in_bridge(sw2, sw_lag_name),\
        "L3 LAG %s should not be part of bridge_normal" % (sw_lag_name)
